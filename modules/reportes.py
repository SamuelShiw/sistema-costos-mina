...
def preparar_resumen(df_agrupado):
    ...
    """
    # Mapeo de columnas internas -> Encabezados Excel
    cols_map = {
    ...
    # Cálculo de métrica derivada (Costo Unitario)
...
def preparar_detalle(df_detalle):
    ...
    cols_map = {
        ...
        'usuario_registro': 'Digitado Por' # Si existe
    }
...
def generar_excel_corporativo(df_detalle, df_resumen, usuario="Admin", rol="Lector"):
    ...
    # Colores Corporativos (Azul Acero)
    ...
    # 2. Creación de Tabla Dinámica (Pivot) Manual para Gráfico
    # Agrupamos por Categoría o Material para el gráfico
    pivot_chart = pd.pivot_table(
        df_det_final,
        values='Costo Total (S/)',
        index='Rubro / Categoría', # Usamos Categoría que es más general
        aggfunc='sum'
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # 4. Estilos y Formato Profesional
        ...
        def formatear_hoja(sheet_name, titulo):
            ...
            # Formato de Tabla (Headers y Celdas)
            max_row = ws.max_row
            ...
            for col in range(1, max_col + 1):
                ...
                # Iteramos filas de datos
                for row in range(6, max_row + 1):
                    cell = ws[f"{col_letter}{row}"]
                    cell.border = border_box
                    
                    # Formato Moneda/Número
        ...
        # Aplicamos formato a las 3 hojas
        ...
        # 5. Gráfico de Barras Incrustado
        ws_pivot = workbook['Análisis Gráfico']
        chart = BarChart()
        chart.type = "bar"
        chart.style = 10
        chart.title = "Gasto Total por Categoría (Pareto)"
        chart.y_axis.title = "Soles (S/)"
        chart.x_axis.title = "Categoría"
        
        # Referencias de datos para el gráfico
        ...
        # Ubicamos el gráfico a la derecha de la tabla
        ws_pivot.add_chart(chart, "E5")

    # Retornamos los bytes del archivo
    output.seek(0)
